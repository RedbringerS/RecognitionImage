...
public class Recognition {
    ...
    public String getRankOrSuit(int number, char type) { //функция, которая возвращает значение значимости
        ...
        if (img1 != null && img1.getRGB(img1.getWidth() - 1, img1.getHeight() - 1) == -8882056) { //если последний пиксель - это серый пиксель (значение -8882056)
        ...
        if (number < 3 || isCard) { //если это одна из первых трех карт или есть тоже четвертая или пятая карта
            File dir = null;
            if (type == 'r') {
    ...
    public boolean isFourThorFifthCard(int r, int g, int b) { //Есть ли четвертая или пятая карта
        //если это белый или серый пиксель
        return (r == 255 && g == 255 && b == 255) || (r == 120 && g == 120 && b == 120); //то есть карта
    }
    ...
    public String getImageAsString(BufferedImage symbol) { //преобразование изображения в бинарную строку
        ...
        //то есть строку, где черному пикселю соответствует '*', а белому — пробел.
    }
        ...
        for (int i = 0; i< img.getWidth(); i++) {
            for (int j = 0; j< img.getHeight(); j++) {
                if (type == 's') { //если это изображение значимости или масти карты
                    if (img.getRGB(i, j) == -8882056) { //если это серый пиксель
                        img.setRGB(i, j, -1); //сохранение как белого пикселя
                    } else {
                        img.setRGB(i, j, 0); //все остальные пиксели соранить как черные
                    }
                }
                else if (type == 't') { //если это изображение-шаблон
                    if (img.getRGB(i, j) != -1) {
                        img.setRGB(i, j, 0); //если пиксель не белый, сохранить как черный
                    }
                }
